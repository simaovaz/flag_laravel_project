        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <input type="text" id="name" class="form-control @error('name') is-invalid  @enderror" name="name"
                    value="{{ $medico->name ?? '' }}" required autofocus>
                @error('name')
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <input type="text" id="address" class="form-control @error('address') is-invalid  @enderror"
                    name="address" value="{{ $medico->address }}" required autofocus>
                @error('address')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <input type="text" id="phone" class="form-control @error('phone') is-invalid  @enderror" name="phone"
                    value="{{ $medico->phone }}" required autofocus>
                @error('phone')
            ...
            <div class="col-md-6">
                <select class="custom-select" name="especialidade">
                    @foreach ($especialidades as $especialidade)
                        <option value="{{ $especialidade->id }}" {{ $medico->speciality_id == $especialidade->id ? 'selected' : 'nada' }}> {{ $especialidade->name }} </option>
                    @endforeach
                </select>
                @error('especialidade')
                    @foreach ($errors->getMessages()['especialidade'] as $message)
                        <li class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </li>
                    @endforeach
